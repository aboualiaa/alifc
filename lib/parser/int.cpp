...
void parser_Int(std::string Token[2048], lex::Token *o_tokens) {
  ...
  if (IsInsideWindow && !IsInsideFunction) {
    ErrorCode("انشاء متغير جديد يجب ان يكون خارج النافذة ليكون عام، او داخل "
              "دالة ليكون محلي",
              o_tokens);
  }
  ...
  if (Token[INT_POS + 1].empty()) {
    ErrorCode("يجب تحديد اسم المتغير", o_tokens);
  }

  if (!Token[INT_POS + 2].empty() && Token[INT_POS + 2] != "=") {
    ErrorCode("يجب استعمال ' = ' بعد اسم المتغير", o_tokens);
  }

  if (Token[INT_POS + 2] == "=" && Token[INT_POS + 3].empty()) {
  ...
  if (IsInsideClass) {
    if (!IsInsideFunction) {
      ...
      if (Token[1] == "خاص") {
        if (!o_tokens->TOKENS_PREDEFINED) {
          SetNewVarClass(true, true, TheClass, TheFunction, Token[INT_POS + 1],
                         "عدد", false, false, o_tokens->Line, o_tokens);
          return; // continue;
        }

        if (DEBUG) {
          DEBUG_MESSAGE("	[PRIVATE] [CLASS-GLOBAL-INT] [" +
                            Token[INT_POS + 1] + "] ",
                        o_tokens); // DEBUG
        }
        ...
        if (DEBUG) {
          DEBUG_MESSAGE("	[CLASS-GLOBAL-INT] [" + Token[INT_POS + 1] +
                            "] ",
                        o_tokens); // DEBUG
        }
      ...
      if (Token[1] == "خاص") {
        ...
        if (DEBUG) {
          DEBUG_MESSAGE("		[CLASS-INT] [" + Token[INT_POS + 1] +
                            "] ",
                        o_tokens); // DEBUG
        }
  ...
  // Global INT
    ...
    if (!o_tokens->TOKENS_PREDEFINED) {
      SetNewVar(true, "", "", Token[INT_POS + 1], "عدد", false, false,
    ...
    if (DEBUG) {
      DEBUG_MESSAGE("[GLOBAL-INT] [" + Token[INT_POS + 1] + "] ",
                    o_tokens); // DEBUG
    }
    ...
    SetNewVar(false, TheWindow, TheFunction, Token[INT_POS + 1], "عدد", false,
              false, o_tokens->Line, o_tokens);

    if (DEBUG) {
      DEBUG_MESSAGE("		[LOCAL-INT] [" + Token[INT_POS + 1] + "] ",
                    o_tokens); // DEBUG
    }
  ...
  if (Token[INT_POS + 2] == "=") {
    ...
    for (int p = (INT_POS + 2); p <= o_tokens->TOTAL[o_tokens->Line]; p++) {
      if (!Token[p].empty()) {
        ...
        {
          if (Token[INT_POS + 1] ==
              substr_utf8(Token[p], 1, CharCount_utf8(Token[p], o_tokens))) {
            ErrorCode("لا يمكن استعمال المتغير العام في نفس السطر الذي تم "
                      "الانشاء فيه '" +
                          Token[p] + "' ",
                      o_tokens);
          }
                    ...
                    ":")) // to avoid a = b + a, but ok for a = b + x:y:a
        {
          if (G_VAR_IS_SET[(Token[INT_POS + 1])] &&
              IntToString(o_tokens->Line) !=
                  G_VAR_AT_LINE[(Token[INT_POS + 1])]) {
            ErrorCode(
                "لا يمكن استعمال المتغير في نفس السطر الذي تم الانشاء فيه '" +
                    Token[p] + "', كذلك اذا كنت تود استعمال المتغير العام '" +
                    Token[INT_POS + 1] +
                    "', يجب اضافه خط قبل اسم المتغير على سبيل المثال : _" +
                    Token[INT_POS + 1],
                o_tokens);
          }
    ...
    // INT Syntax
    ScriptSyntaxBuffer =
        CheckForSyntax("عدد",     // OBJECTIF_TYPE
