...
void parser_TwoPointOperator(std::string Token[2048], lex::Token *o_tokens) {
  // :
  ...
  if (CONTROL_WIN_IS_SET[Token[1]] || WIN_IS_SET[Token[1]] ||
    ...
    if (Token[4] == "(") {
      ...
      // -----------------------
        ...
        TempTokenCount = 0;
        for (int p = 3; p <= o_tokens->TOTAL[o_tokens->Line];
             p++) // | a, b + 2, c) |
        {
          if (Token[p] != "") {
    ...
    if (Token[4] == "=") {
      ...
      else if (Token[3] == "نص")
      {
              ...
              if(DEBUG)DEBUG_MESSAGE("		[WIN'" + Token[1] + " ':VALUE'"
      + Token[3] + "(SetWindowLabel)' = ", o_tokens); // DEBUG
      ...
      ")); else
                      // Window : Text =
                      cpp_AddScript(TheFunction, CG_WIN_MEMBER(Token[1],
      "SetLabel( "));
              // *** *** *** *** *** ***
              VALUE_CPP_END = " ); \n";
              // *** *** *** *** *** ***
      ...
      */
        ...
        ErrorCode("النافذة ' " + Token[1] + " ' لا تحتوي على خاصيه باسم ' " +
                      Token[3] + " ' ",
                  o_tokens);
      }
      ...
      TempTokenCount = 0;
      for (int p = 4; p <= o_tokens->TOTAL[o_tokens->Line];
           p++) // | = a * b + 2 / (c) + 1 |
      {
        if (Token[p] != "") {
          TempToken[TempTokenCount] = Token[p];
  ...
  ScriptSyntaxBuffer =
      CheckForSyntax(CTR_OPTION_TYPE, // OBJECTIF_TYPE
                     true,      // Accept Using Reference to Window:Controls
                     true,      // Accept Using Reference to Window:Function
                     true,      // Accept Using Reference to Global Functions
                     true,      // Accept Using Reference to Local Functions
