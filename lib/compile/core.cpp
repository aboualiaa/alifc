...
void CheckForSameGlobalID(const std::string &Name, lex::Token *o_tokens) {
  ...
  if (OBJ_IS_SET[std::make_pair("", Name)]) {
    ...
    ErrorCode(
        "تشابه في الاسم مع نافذة ثم إنشائها مسبقا، المرجو تغيير الاسم : ' " +
            Name + " ' ",
        o_tokens);
  }
}
...
void SetNewVar(bool IsGlobal, const std::string &TmpWindow,
  ...
  if (IsGlobal) {
    ...
    if (L_VAR_IS_SET[std::make_pair(TmpWindow + TmpFunction, VarName)]) {
      ErrorCode(
          "المتغير ' " + VarName + " ' تم انشاؤه مسبقا في السطر : " +
              L_VAR_AT_LINE[std::make_pair(TmpWindow + TmpFunction, VarName)],
          o_tokens);
    }
    ...
    L_VAR_AT_LINE[std::make_pair(TmpWindow + TmpFunction, VarName)] =
        IntToString(Line);
    L_VAR_IS_CONST[std::make_pair(TmpWindow + TmpFunction, VarName)] =
        IsConstant;
...
auto CHECK_NEW_FUN_SYNTAX(bool GLOBAL, std::string SYNTAX[1024],
  ...
  for (int p = 0; p <= SYNTAX_LONG; p += 3) {
    if (!o_tokens->TOKENS_PREDEFINED) {
      ...
      SetNewVar(false, TmpWindow, TmpFunction, SYNTAX[VAR], SYNTAX[TYPE], false,
...
auto CheckForSyntax(
  ...
  for (int p = 1; p <= SYNTAX_LONG; p++) {
    ...
    if (SYNTAX[p] == "_ج_") {
      ErrorCode("لا يمكن بأي شكل من الأشكال معرفة نوع البيانات الناتجة من شفرة "
                "جافا سكريبت عشوائية",
                o_tokens);
    ...
    else if (OBJ_IS_SET[std::make_pair(TMP_WIN_OR_CLASS + TmpFunction,
                                       SYNTAX[p])] ||
    ...
    else if (CLASS_G_VAR_IS_SET[std::make_pair(TMP_WIN_OR_CLASS, SYNTAX[p])] &&
             IsInsideClass) {
      // Class Global area
    ...
    */
      ...
      if (OBJECTIF_TYPE == "عدد") {
        if (FUN_TYPE == "عدد") {
          if (IS_LOCAL_FUN) {
            ...
            // *** Generate Code ***
            // Buffer
            CG_BUFFER.append(" OBJ_CLASS_WINDOW_" + ID[TheWindow] +
                             "->FUNCTION_" + ID[SYNTAX[p]] + "( ");
            // *** *** *** *** *** ***

            CG_BUFFER.append(CHECK_CALL_FUN_ARG(
                false, TMP_WIN_OR_CLASS, SYNTAX[p], 0, TMP_WIN_OR_CLASS,
            ...
            CG_BUFFER.append(CHECK_CALL_FUN_ARG(
                true, "", SYNTAX[p], 0, TMP_WIN_OR_CLASS, TheFunction,
        ...
        if (FUN_TYPE == "عدد") {
          if (IS_LOCAL_FUN) {
            ...
            CG_BUFFER.append(" alifcore_IntToString( OBJ_CLASS_WINDOW_" +
                             ID[TheWindow] + "->FUNCTION_" + ID[SYNTAX[p]] +
                             "( ");
            // *** *** *** *** *** ***
        ...
        if (IS_LOCAL_FUN) {
          ...
          CG_BUFFER.append(CHECK_CALL_FUN_ARG(
              true, "", SYNTAX[p], 0, TMP_WIN_OR_CLASS, TheFunction, TempToken,
    ...
    // -----------------------------------
             ...
             CONTROL_IS_SET[std::make_pair(TMP_WIN_OR_CLASS, SYNTAX[p])]) {
      ...
      if (CONTROL_WIN_IS_SET[SYNTAX[p]] || WIN_IS_SET[SYNTAX[p]]) {
        ...
        if (SYNTAX[p + 3] == "(") {
          ...
          if (SYNTAX[p + 2] == "إظهار" || SYNTAX[p + 2] == "إخفاء" ||
              SYNTAX[p + 2] == "تدمير") {
            ErrorCode("لا يمكن إستعمال خاصية ' " + SYNTAX[p + 2] +
                          " ' هنا، لأنها من نوع عادم" + OBJECTIF_TYPE,
                      o_tokens);
          }
          ...
          // -----------------------
            ...
            if (OBJECTIF_TYPE == "عدد") {
              if (FUN_TYPE == "عدد") {
                ...
                // Buffer
                CPP_END = " ";
                CPP_CODE.append(" OBJ_CLASS_WINDOW_" + ID[SYNTAX[p]] +
        ...
        ---------------------------------------------------------------------------------
                ...
                if (SYNTAX[p + 2] == "عنوان")
                {
                        ...
                        if(DEBUG)DEBUG_MESSAGE("		[WIN'" +
        SYNTAX[p] + " ':VALUE'" + SYNTAX[p + 2] + "(SetWindowTitle)' = ",
        o_tokens); // DEBUG
        ...
        CG_WIN_MEMBER(SYNTAX[p], "SetLabel (wxT( "));
                        // *** *** *** *** *** ***
                        VALUE_CPP_END = " )); \n } \n";
                        // *** *** *** *** *** ***
                }
        ...
        خاصيه باسم ' " + SYNTAX[p + 2] + " ' ", o_tokens);
                }
        ...
        = a * b + 2 / (c) + 1 |
                ...
                CheckForSyntax(VALUE_TYPE,	// OBJECTIF_TYPE
                                                true, 		// Accept Using
        Reference to Window:Controls
                                                true, 		// Accept Using
        Reference to Window:Function
                                                true, 		// Accept Using
        Reference to Global Functions
                                                true, 		// Accept Using
        Reference to Local Functions
                                                true, 		// Accept Using
        Reference to Global VAR true, 		// Accept Using Reference to
        Local VAR false, 		// Accept Convertion from String To Int
                                                true, 		// Accept
        Convertion from Int To String
                                                TempToken,
        // SYNTAX[] string (TempTokenCount - 1),	// SYNTAX_LONG int
                                                TheWindow,		//
        TMP_WINDOW_NAME TheFunction,		// TMP_FUNCTION_NAME o_tokens);
        ...
        */
        ...
        // if (SYNTAX[p + 4] == "")
        // ErrorCode("يجب اضافة قيمة بعد ' " + SYNTAX[p] + " : " + SYNTAX[p + 2]
        // + " " + SYNTAX[p + 3] + " ' ", o_tokens);
      ...
      if (CTR_OPTION == "نص") // abc = Control:نص
      {
        ...
        if (OBJECTIF_TYPE == "عدد") {
          ...
          CPP_CODE.append(" (alifcore_StringToInt(OBJ_CTR_" + ID[CTR_WIN] +
                          "_" + Control_ID[CTR_CONTROL] + "->" + GetValueFix +
                          "())) ");
          // *** *** *** *** *** ***
        } else if (OBJECTIF_TYPE == "نص") {
          if (DEBUG) {
    ...
    // -----------------------------------
      ...
      // أو
      else if (SYNTAX[p] == "او" || SYNTAX[p] == "ٱو" || SYNTAX[p] == "آو" ||
...
bool IsValidName(string Name)
{
        ...
        while (I < CharCount_utf8(Name))
        {
                        ...
                        Char == "۹")
                {
                        if (First)
                        {
                                ////cout << "VALID_NAME - false.First : |" <<
Char << "|" << endl; return false;
                        }
                }
