...
void parser_Class(std::string Token[2048], CLASS_TOKEN *o_tokens) {
  ...
  if (IsInsideClass) {
    ErrorCode("لا يمكن إنشاء صنف داخل صنف، الصنف الحالي  : " + TheClass,
              o_tokens);
  }

  if (IsInsideFunction) {
    ErrorCode("لا يمكن أنشاء صنف داخل دالة، يجب إنشائها في مجال عام فقط",
              o_tokens);
  }

  if (IsInsideWindow) {
    ErrorCode("لا يمكن أنشاء صنف داخل نافذة، يجب إنشائها في مجال عام فقط",
              o_tokens);
  }
  ...
  // TODO(aboualiaa): Check if extended from other class

  if (!o_tokens->TOKENS_PREDEFINED) {
    if (CLASS_IS_SET[Token[2]]) {
      ErrorCode("الصنف ' " + Token[2] + " ' تم انشاؤه مسبقا في السطر رقم : " +
                    CLASS_AT_LINE[Token[2]],
                o_tokens);
    }
  ...
  // TheWindow = TheClass; // < ======================================= TODO: TO
  // BE CHECK IF THIS WORKAROUND WORK FINE !!!!!!

  if (DEBUG) {
    DEBUG_MESSAGE("[NEW CLASS] [" + Token[2] + "] \n\n", o_tokens); // DEBUG
  }
