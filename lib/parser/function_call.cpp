...
void parser_FunctionCall(std::string Token[2048], lex::Token *o_tokens) {
  ...
  if (CallGlobalFun) {
    ...
    // Call a Local func
    ScriptBuffer = " OBJ_CLASS_WINDOW_" + ID[TheWindow] + "->FUNCTION_" +
                   ID[FunName] + "(" + ScriptSyntaxBuffer + "); \n";
  }
  ...
  if (L_FUN_IS_SET[std::make_pair(TheWindow, Token[1])])
  {
          ...
          if (L_FUN_TYPE[std::make_pair(TheWindow, Token[1])] == "عدد")
                  ErrorCode("الدالة المحلية ' " + Token[1] + "()' من نوع عدد,
  لذلك أنت بحاجة إلى متغير للحصول على قيمة الرجوع", o_tokens);

          if (L_FUN_TYPE[std::make_pair(TheWindow, Token[1])] == "نص")
                  ErrorCode("الدالة المحلية ' " + Token[1] + "()' من نوع كلمة,
  لذلك أنت بحاجة إلى متغير للحصول على قيمة الرجوع", o_tokens);

          if(DEBUG)DEBUG_MESSAGE("		[CALL-LOCAL-FUNCTION ' " +
  Token[1] + " '] (", o_tokens); // DEBUG

          // *** Generate Code ***
          // Call Local Func from inside Local Func
          cpp_AddScript(TheFunction, " OBJ_CLASS_WINDOW_" + ID[TheWindow] +
  "->FUNCTION_" + ID[Token[1]] + " (");
          // *** *** *** *** *** ***

          // Check local fun Args and Syntax : fun (a + c, 2 * (b - 1))
          ScriptSyntaxBuffer = CHECK_CALL_FUN_ARG(false, TheWindow, Token[1], 0,
  TheWindow, TheFunction, TempToken, (TempTokenCount - 1),  o_tokens);
  ...
  else if (G_FUN_IS_SET[(Token[1])])
  {
          ...
          if (G_FUN_TYPE[(Token[1])] == "عدد")
                  ErrorCode("الدالة العامه ' " + Token[1] + "()' من نوع عدد,
  لذلك أنت بحاجة إلى متغير للحصول على قيمة الرجوع", o_tokens);

          if (G_FUN_TYPE[(Token[1])] == "نص")
                  ErrorCode("الدالة العامه ' " + Token[1] + "()' من نوع كلمة,
  لذلك أنت بحاجة إلى متغير للحصول على قيمة الرجوع", o_tokens);

          if (IsInsideClass)
          {
                  if(DEBUG)DEBUG_MESSAGE("		[CALL-GLOBAL-FUNCTION '
  "
  + Token[1] + " '] ( ", o_tokens); // DEBUG
  ...
  " (");
          ...
          ScriptSyntaxBuffer = CHECK_CALL_FUN_ARG(true, "", Token[1], 0,
