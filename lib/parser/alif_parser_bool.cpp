...
void parser_bool(string Token[2048], CLASS_TOKEN *o_tokens) {
  ...
  if (IsInsideWindow && !IsInsideFunction)
    ErrorCode("انشاء متغير منطقي جديد يجب ان يكون خارج النافذة ليكون عام، او "
              "داخل دالة ليكون محلي",
              o_tokens);
  ...
  if (IsInsideClass) {
    if (!IsInsideFunction) {
      ...
      if (Token[1] == "خاص") {
        ...
        if (DEBUG)
          DEBUG_MESSAGE("	[PRIVATE] [CLASS-GLOBAL-BOOL] [" +
                            Token[BOOL_POS + 1] + "] ",
                        o_tokens); // DEBUG
        ...
        if (DEBUG)
          DEBUG_MESSAGE("	[CLASS-GLOBAL-BOOL] [" + Token[BOOL_POS + 1] +
                            "] ",
                        o_tokens); // DEBUG
      ...
      if (Token[1] == "خاص") {
        ...
        if (DEBUG)
          DEBUG_MESSAGE("		[CLASS-BOOL] [" + Token[BOOL_POS + 1] +
                            "] ",
                        o_tokens); // DEBUG
    ...
    if (DEBUG)
      DEBUG_MESSAGE("[GLOBAL-BOOL] [" + Token[BOOL_POS + 1] + "] ",
                    o_tokens); // DEBUG
    ...
    SetNewVar(false, TheWindow, TheFunction, Token[BOOL_POS + 1], "منطق", false,
              false, o_tokens->Line, o_tokens);

    if (DEBUG)
      DEBUG_MESSAGE("		[LOCAL-BOOL ] [" + Token[BOOL_POS + 1] + "] ",
                    o_tokens); // DEBUG
      ...
      if (Token[p] != "") {
            ...
            !IsInsideFunction) // to avoid : a = b + _a
        {
          if (Token[BOOL_POS + 1] ==
            ...
            ErrorCode("لا يمكن استعمال المتغير المنطقي العام في نفس السطر الذي "
                      "تم الانشاء فيه '" +
                          Token[p] + "' ",
                      o_tokens);
                    ...
                    ":")) // to avoid a = b + a, but ok for a = b + x:y:a
        {
          if (G_VAR_IS_SET[(Token[BOOL_POS + 1])] &&
              IntToString(o_tokens->Line) !=
                  G_VAR_AT_LINE[(Token[BOOL_POS + 1])])
            ErrorCode(
                "لا يمكن استعمال المتغير المنطقي في نفس السطر الذي تم الانشاء "
                "فيه '" +
          ...
          ErrorCode("لا يمكن استعمال المتغير المنطقي في نفس السطر الذي تم "
                    "الانشاء فيه '" +
                        Token[p] + "' ",
                    o_tokens);
        }
