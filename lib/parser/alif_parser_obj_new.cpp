...
void parser_ObjNew(std::string Token[2048], CLASS_TOKEN *o_tokens) {
  ...
  // OBJ_AT_LINE[std::make_pair(TheWindow + TheFunction, Token[3])] =
  // IntToString(o_tokens->Line); OBJ_CLASS[std::make_pair(TheWindow +
  // TheFunction, Token[3])] = Token[2];
  ...
  if (IsInsideWindow) {
    ...
    if (!o_tokens->TOKENS_PREDEFINED) {
      ...
      // CPP_CLASS_GLOBAL_OBJ.append(" CLASS_" + Global_ID[Token[4]] + " " +
      // GlobalObj_ID[Token[2]] + " ");
