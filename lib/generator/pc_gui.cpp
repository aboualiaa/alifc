...
void Script(string Code){
        ...
        else
        {
                ...
                else
                {
                        ...
                        CPP_WINDOW[std::make_pair(TheWindow, "LOAD")] = CBUFER +
NEW_CODE;
                }
        }
}
...
// ------------------------------------------------
// Window X )" +
             ID[WIN_TOTAL_NAMES[i]] + R"( Load()
// ------------------------------------------------

void WINDOW_LOAD_)" +
             ID[WIN_TOTAL_NAMES[i]] + R"(()
{
	)" + CPP_WINDOW[std::make_pair(WIN_TOTAL_NAMES[i], "LOAD")] +
             R"(
}
// ========================================================================
...
auto CG_APP_DESTROY() -> std::string {
  ...
  for (int i = 1; i <= WIN_TOTAL; i++) {
    BUFFER = R"(
		// Window X )" +
             ID[WIN_TOTAL_NAMES[i]] + R"( Destroy
		// ---------
		//#ifdef __APPLE__
			//if (WINDOW_IS_CONSTRUCTION_)" +
             ID[WIN_TOTAL_NAMES[i]] + R"() OBJ_CLASS_WINDOW_)" +
             ID[WIN_TOTAL_NAMES[i]] + R"(->EndModal(0);
		//#else
			if (WINDOW_IS_CONSTRUCTION_)" +
             ID[WIN_TOTAL_NAMES[i]] + R"() OBJ_CLASS_WINDOW_)" +
             ID[WIN_TOTAL_NAMES[i]] + R"(->Destroy();
...
auto AlifJavaScript_Bridge_SendRequest() -> std::string {
  ...
  return " window.open(link); ";
#else
  return " document.title = link; window.open('http://AlifJSBridge'); "
         "document.title = ''; ";
#endif
}
...
auto Generate_AlifJavaScript() -> std::string {
  ...
  for (int g = 0; g < G_VAR_TOTAL; g++) {
    ...
    if (GlobalVarType == "منطق") {
      ...
      Return = "this->AlifJavaScript_Error(wxT(\"علة. نوع المتغير غير "
               "معروف\"));return; // ";
    }

    ScriptBuffer_GlobalVarsCallCondition.append(R"V0G0N(
		else if (Tokens[0] == wxT(")V0G0N" +
                                                GlobalVarName + R"V0G0N(")){
			...
			)V0G0N" + Return + R"V0G0N( )V0G0N" +
                                                Global_ID[GlobalVarName] +
                                                Return_end + R"V0G0N(
  ...
  for (int f = 0; f < Global_TotalFucntion; f++) {
    ...
    if (FunctionType == "عادم") {
      ...
      Return = "this->AlifJavaScript_Error(wxT(\"علة. نوع الدالة غير "
               "معروف\"));return; // ";
    }
    ...
    for (int a = 0; a < ARG_Total; a++) { // arg loop
      ...
      if (Type == "عدد") {
        ...
        Return = "this->AlifJavaScript_Error(wxT(\"علة. نوع معطيات غير "
                 "معروف\"));return; // ";
      }
    ...
    ScriptBuffer_GlobalFuncCallCondition.append(
        ...
        FunctionName + R"V0G0N(")){

			if(PramsCount != )V0G0N" +
        FunctionARGS +
        R"V0G0N( ){this->AlifJavaScript_Error(wxT("الدالة العامة تقبل فقط )V0G0N" +
        FunctionARGS + R"V0G0N( معطيات"));return;}

			)V0G0N" +
        Return + R"V0G0N( FUNCTION_)V0G0N" + Global_ID[FunctionName] +
        R"V0G0N( ( )V0G0N" + ARG_Call + R"V0G0N( ) )V0G0N" + Return_end +
        R"V0G0N(
  ...
  for (int i = 0; i <= WIN_TOTAL; i++) {
    ...
    FunctionName = "";
    WebUI_obj_name =
        "OBJ_CTR_" + ID[WindowName] + "_" + Control_ID["AlifUIWeb"];

    // OnNavigationRequest()
    ScriptBuffer.append(
        R"V0G0N(
			void CLASS_WINDOW_)V0G0N" +
        ID[WindowName] +
        R"V0G0N(::AlifJavaScript_OnNavigationRequest(wxWebViewEvent& evt){
				evt.Veto();
    ...
    ScriptBuffer.append(
        R"V0G0N(
			void CLASS_WINDOW_)V0G0N" +
        ID[WindowName] +
        R"V0G0N(::AlifJavaScript_OnNewWindow(wxWebViewEvent& evt){
				)V0G0N" +
        AlifJavaScript_Bridge_OnNewWindowSendRequest(WebUI_obj_name) + R"V0G0N(
			}  
		)V0G0N");
    ...
    ScriptBuffer.append(
        R"V0G0N(
			void CLASS_WINDOW_)V0G0N" +
        ID[WindowName] + R"V0G0N(::AlifJavaScript_Error(wxString sError){
				)V0G0N" +
        WebUI_obj_name +
        R"V0G0N(->RunScript(wxT("function fJSTempBuffer(){AlifJSBridg_Res='" + sError +"';AlifJSBridg_Status=false;}fJSTempBuffer();"));
			}  
		)V0G0N");
    ...
    ScriptBuffer.append(
        R"V0G0N(
			void CLASS_WINDOW_)V0G0N" +
        ID[WindowName] + R"V0G0N(::AlifJavaScript_Run(wxString JSCode){
				)V0G0N" +
        WebUI_obj_name +
        R"V0G0N(->RunScript(wxT("function fAlifJSRun(){" + JSCode + "}fAlifJSRun();"));
			}  
		)V0G0N");
    ...
    for (int f = 0; f < Win_TotalFucntion[WindowName]; f++) {
      ...
      for (int a = 0; a < ARG_Total; a++) { // arg loop
        ...
        if (Type == "عدد") {
          ...
          ARG_Call.append(" alifcore_StringToBool(Tokens[" +
                          IntToString(a + 1) + "]) ");
        } else {
          Return = "this->AlifJavaScript_Error(wxT(\"علة. نوع معطيات غير "
                   "معروف\"));return; // ";
      ...
      ScriptBuffer_FunctionCallCondition.append(
          ...
          FunctionName + R"V0G0N(")) || (Tokens[0] == wxT(")V0G0N" +
          WindowName + R"V0G0N(:)V0G0N" + FunctionName + R"V0G0N("))){

				if(PramsCount != )V0G0N" +
          FunctionARGS +
          R"V0G0N( ){this->AlifJavaScript_Error(wxT("الدالة تقبل فقط )V0G0N" +
          FunctionARGS + R"V0G0N( معطيات"));return;}

				)V0G0N" +
          Return + R"V0G0N( FUNCTION_)V0G0N" + ID[FunctionName] +
          R"V0G0N( ( )V0G0N" + ARG_Call + R"V0G0N( ) )V0G0N" + Return_end +
          R"V0G0N(
    ...
    ScriptBuffer.append(
        R"V0G0N(
			void CLASS_WINDOW_)V0G0N" +
        ID[WindowName] + R"V0G0N(::AlifJavaScriptBridge(wxString sLink){
        ...
        ScriptBuffer_FunctionCallCondition +
        ScriptBuffer_GlobalFuncCallCondition +
        ScriptBuffer_GlobalVarsCallCondition + R"V0G0N(
				else {
        ...
        WebUI_obj_name +
        R"V0G0N(->RunScript(wxT("function fJSBuffer(){AlifJSBridg_Res=" + FunctionReturn +";AlifJSBridg_Status=true;}fJSBuffer();"));
			}  
		)V0G0N");
...
// --------------------
// Window )" +
        ID[WIN_TOTAL_NAMES[i]] + R"( Definition
// --------------------

class CLASS_WINDOW_)" +
        ID[WIN_TOTAL_NAMES[i]] + R"( : public wxFrame
{
    public:
		void OnClose(wxCloseEvent& event);
        CLASS_WINDOW_)" +
        ID[WIN_TOTAL_NAMES[i]] + R"( ();
        virtual ~CLASS_WINDOW_)" +
        ID[WIN_TOTAL_NAMES[i]] + R"(();
		
		// --------------------
		// Window )" +
        ID[WIN_TOTAL_NAMES[i]] + R"( Functions Declaration
		// --------------------
    ...
    private:
		DECLARE_NO_COPY_CLASS(CLASS_WINDOW_)" +
        ID[WIN_TOTAL_NAMES[i]] + R"()
        DECLARE_EVENT_TABLE()
};
...
CLASS_WINDOW_)" +
        ...
        ID[WIN_TOTAL_NAMES[i]] + R"(;
CLASS_WINDOW_)" +
        ID[WIN_TOTAL_NAMES[i]] + R"(::~CLASS_WINDOW_)" +
        ID[WIN_TOTAL_NAMES[i]] + R"((){}

// --------------------
// Window )" +
        ID[WIN_TOTAL_NAMES[i]] + R"( Constructor
// --------------------
...
CLASS_WINDOW_)" +
	...
	wxFrame(NULL, ID_WINDOW_)" +
        ID[WIN_TOTAL_NAMES[i]] + R"(,
	wxT(")" +
        CPP_WINDOW[std::make_pair(WIN_TOTAL_NAMES[i], "نص")] + R"("),
	wxPoint()" +
        CPP_WINDOW[std::make_pair(WIN_TOTAL_NAMES[i], "افصول")] + R"(, )" +
        CPP_WINDOW[std::make_pair(WIN_TOTAL_NAMES[i], "ارتوب")] + R"(),
	wxSize()" +
        CPP_WINDOW[std::make_pair(WIN_TOTAL_NAMES[i], "عرض")] + R"(, )" +
        CPP_WINDOW[std::make_pair(WIN_TOTAL_NAMES[i], "ارتفاع")] + R"(), )" +
...
void CLASS_WINDOW_)" +
        ID[WIN_TOTAL_NAMES[i]] + R"(::OnClose(wxCloseEvent& event)
{
	event.Skip();

	WINDOW_IS_SHOW_)" +
        ID[WIN_TOTAL_NAMES[i]] + R"( = false;
	WINDOW_IS_CONSTRUCTION_)" +
        ID[WIN_TOTAL_NAMES[i]] + R"( = false;
...
BEGIN_EVENT_TABLE(CLASS_WINDOW_)" +
        ID[WIN_TOTAL_NAMES[i]] + R"(, wxFrame)
	EVT_CLOSE(CLASS_WINDOW_)" +
        ID[WIN_TOTAL_NAMES[i]] + R"(::OnClose)
...
int ID_WINDOW_)" +
         ID["رئيسية"] + R"( = ALIFCORE_ID_GENERATOR(); // FIX
...
bool WINDOW_IS_SHOW_)" +
         ID["رئيسية"] + R"( = false; // FIX
bool WINDOW_IS_CONSTRUCTION_)" +
         ID["رئيسية"] + R"( = false; // FIX
void WINDOW_LOAD_)" +
         ID["رئيسية"] + R"((); // FIX
...
// ========================================================================
// Window Main )" +
         ID["رئيسية"] + R"( Fixed Code
// ========================================================================

// ------------------------------------------------
// Window )" +
         ID["رئيسية"] + R"( Definition
// ------------------------------------------------

class CLASS_WINDOW_)" +
         ID["رئيسية"] + R"( : public wxFrame
{
    public:
		void OnClose(wxCloseEvent& event);
        CLASS_WINDOW_)" +
		...
		// --------------------
		// Window )" +
         ID["رئيسية"] + R"( Functions Declaration
		// --------------------
...
CLASS_WINDOW_)" +
         ID["رئيسية"] + R"( *OBJ_CLASS_WINDOW_)" + ID["رئيسية"] + R"(;
CLASS_WINDOW_)" +
         ID["رئيسية"] + R"(::~CLASS_WINDOW_)" + ID["رئيسية"] + R"((){}

// --------------------
// Window )" +
         ID["رئيسية"] + R"( Constructor
// --------------------
...
CLASS_WINDOW_)" +
	...
	wxPoint()" +
         CPP_WINDOW[std::make_pair("رئيسية", "افصول")] + R"(, )" +
         CPP_WINDOW[std::make_pair("رئيسية", "ارتوب")] + R"(),
	wxSize()" +
         CPP_WINDOW[std::make_pair("رئيسية", "عرض")] + R"(, )" +
         CPP_WINDOW[std::make_pair("رئيسية", "ارتفاع")] + R"(), )" +
         CPP_WINDOW[std::make_pair("رئيسية", "شكل")] + R"( )
{
	...
	// --------------------
	// Window )" +
         ID["رئيسية"] + R"( Controls Constructor
	// --------------------
...
bool MyApp::OnInit()
{
	...
	OBJ_CLASS_WINDOW_)" +
         ID["رئيسية"] + R"( = new CLASS_WINDOW_)" + ID["رئيسية"] + R"(();
	OBJ_CLASS_WINDOW_)" +
         ID["رئيسية"] + R"(->SetLayoutDirection(wxLayout_RightToLeft);
    OBJ_CLASS_WINDOW_)" +
         ID["رئيسية"] + R"(->Show();
	SetTopWindow(OBJ_CLASS_WINDOW_)" +
         ID["رئيسية"] + R"();
	OBJ_CLASS_WINDOW_)" +
         ID["رئيسية"] + R"(->Refresh();
	WINDOW_IS_SHOW_)" +
...
void CLASS_WINDOW_)" +
         ID["رئيسية"] + R"(::OnClose(wxCloseEvent& event)
{
	event.Skip();
