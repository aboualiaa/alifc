...
while (std::getline(isCode, Line)){
          ...
          if (p != std::string::npos){
                  ...
                  //ErrorCode(" ####### Script : ||" + AlifScript + "||",
o_tokens); } else { FormatedHTML.append(Line);
          }
}
  */
  ...
  while (std::getline(ScriptStream, Line)) {
    ...
    (void)p_2; // Just to skip --> warning: variable ‘p_2’ set but not used
    ...
    if (p_1 != std::string::npos) {
      ...
      */
      ...
      if (p_2 != std::string::npos) {
        ...
        // -> image/bmp .gif
        // -> image/gif .ico / .cur
        // -> image/x-icon .jpg / .jpeg / .jfif / .pjpeg / .pjp	-> image/jpeg
        // .png -> image/png .svg
        ...
        std::string DataBase64 = base64_encode(
            reinterpret_cast<const unsigned char *>(FileData.c_str()),
            FileData.length());
        // ---------------------------------------------

        boost::algorithm::replace_first(
            Line, c_URI, ("data:" + FileType + ";base64," + DataBase64));

        // if(DEBUG)DEBUG_MESSAGE("=======> c_URI |" + c_URI + "| \n\n",
        // o_tokens); // DEBUG if(DEBUG)DEBUG_MESSAGE("=======> FileType |" +
        // FileType + "| \n\n", o_tokens); // DEBUG
  ...
  if (!replace_end(FormatedHTML, "</html>", AlifJavaScript_Bridge)) {
    ErrorCode("شيفرة واجهة ويب، لا تحتوي على '</html>' -> " + WindowName,
              o_tokens);
  }

  // --- Bin2C Data ------------------------------
  // BinaryToC() work.. but result need extra work.
  // BinaryToC("D:\\Code\\Alif_App\\Alif_2\\test.jpg",
  // "D:\\Code\\Alif_App\\Alif_2\\test.jpg.cpp");
  // BinaryToC("D:\\Code\\Alif_App\\Alif_2\\test2.png",
  // "D:\\Code\\Alif_App\\Alif_2\\test2.png.cpp");
  // ---------------------------------------------
...
void parser_NewWindowWeb(std::string Token[2048], lex::Token *o_tokens) {
  ...
  if (Token[3] == "رئيسية") {
    ...
    CONTROL_WIN_AT_LINE[(Token[3])], o_tokens);
    }
    */
    ...
    CPP_WINDOW[std::make_pair("رئيسية", "CTR_CONSTRUCTOR")] =
        "wxBoxSizer* o_TopSizer = new wxBoxSizer(wxVERTICAL); "
        "P->SetSizer(o_TopSizer); " +
        CBUFER_OBJ +
        " = wxWebView::New(P, wxID_ANY, \"\", wxPoint(10,10), wxSize(100, "
        "100), wxWebViewBackendDefault, wxBORDER_NONE, "
        "wxT(\"Alif_Application\")); o_TopSizer->Add(" +
        CBUFER_OBJ + ", wxSizerFlags().Expand().Proportion(1)); " + CBUFER_OBJ +
        "->RegisterHandler(wxSharedPtr<wxWebViewHandler>(new "
        "wxWebViewArchiveHandler(\"wxfs\"))); " +
        CBUFER_OBJ +
        "->RegisterHandler(wxSharedPtr<wxWebViewHandler>(new "
        "wxWebViewFSHandler(\"memory\")));\n #include \"" +
        PATH_FULL_HTML_CPP + "\" \n " + CBUFER_OBJ +
        "->LoadURL(\"memory:" + CBUFER_VarName +
        ".htm\"); \n Center(); \n Bind(wxEVT_WEBVIEW_NEWWINDOW, "
        "&CLASS_WINDOW_" +
        ID["رئيسية"] + "::AlifJavaScript_OnNewWindow, this, " + CBUFER_OBJ +
        "->GetId()); \n Bind(wxEVT_WEBVIEW_NAVIGATING, &CLASS_WINDOW_" +
        ID["رئيسية"] + "::AlifJavaScript_OnNavigationRequest, this, " +
        CBUFER_OBJ + "->GetId()); \n " + CBUFER_OBJ +
    ...
    CPP_WINDOW[std::make_pair(TheWindow, "FUN_DECLARATION")] =
        CBUFER + " \n void AlifJavaScript_OnNewWindow(wxWebViewEvent& evt); \n "
                 "void AlifJavaScript_OnNavigationRequest(wxWebViewEvent& "
                 "evt); \n void AlifJavaScript_Error(wxString sError); void "
                 "AlifJavaScript_Run(wxString JSCode); \n void "
                 "AlifJavaScriptBridge(wxString sLink); \n ";
    ...
    // y = ارتوب
    /*
    CPP_WINDOW[std::make_pair(TheWindow, "نص")] = " (" + TheWindow + ") نافدة
    بدون عنوان"; #ifdef __APPLE__ CPP_WINDOW[std::make_pair(TheWindow, "افصول")]
    = "10"; CPP_WINDOW[std::make_pair(TheWindow, "ارتوب")] 	= "25"; //
    Because the macOS task-bar is on top screen. #else
    ...
    CPP_ID_DECLARATION.append(" int ID_WINDOW_" + ID[TheWindow] + " =
    ALIFCORE_ID_GENERATOR(); \n"); CPP_GLOBAL.append(" bool WINDOW_IS_SHOW_" +
    ID[TheWindow] + " = false; \n"); CPP_GLOBAL.append(" bool
    WINDOW_IS_CONSTRUCTION_" + ID[TheWindow] + " = false; \n");
    ...
    */
    ...
    CPP_WINDOW[std::make_pair(Token[3], "CTR_CONSTRUCTOR")] =
        ...
        ".htm\"); \n Center(); \n Bind(wxEVT_WEBVIEW_NEWWINDOW, "
        "&CLASS_WINDOW_" +
        ID[Token[3]] + "::AlifJavaScript_OnNewWindow, this, " + CBUFER_OBJ +
        "->GetId()); \n Bind(wxEVT_WEBVIEW_NAVIGATING, &CLASS_WINDOW_" +
        ID[Token[3]] + "::AlifJavaScript_OnNavigationRequest, this, " +
        CBUFER_OBJ + "->GetId()); \n " + CBUFER_OBJ +
        "->EnableContextMenu(false); \n " + CBUFER_OBJ +
        "->EnableHistory(false); \n ";
    ...
    CPP_WINDOW[std::make_pair(TheWindow, "نص")] =
        " (" + TheWindow + ") نافدة بدون عنوان";
#ifdef __APPLE__
    CPP_WINDOW[std::make_pair(TheWindow, "افصول")] = "10";
    CPP_WINDOW[std::make_pair(TheWindow, "ارتوب")] =
    ...
    CPP_GLOBAL.append(" bool WINDOW_IS_CONSTRUCTION_" + ID[TheWindow] +
                      " = false; \n");
    CPP_GLOBAL.append(" void WINDOW_LOAD_" + ID[TheWindow] + "(); \n");
    // *** *** *** *** *** ***
...
void parser_NewWindow(std::string Token[2048], lex::Token *o_tokens) {
  ...
  } else // نافدة MyWindowName
  {
    ...
    if (CONTROL_WIN_IS_SET[Token[2]]) {
      ...
      // x = افصول
      // y = ارتوب
      CPP_WINDOW[std::make_pair(Token[2], "نص")] =
          " (" + Token[2] + ") نافدة بدون عنوان";
#ifdef __APPLE__
      CPP_WINDOW[std::make_pair(Token[2], "افصول")] = "10";
      CPP_WINDOW[std::make_pair(Token[2], "ارتوب")] =
          "25"; // Because the macOS task-bar is on top screen.
#else
      ...
      CPP_ID_DECLARATION.append(" int ID_WINDOW_" + ID[Token[2]] +
                                " = ALIFCORE_ID_GENERATOR(); \n");
      CPP_GLOBAL.append(" bool WINDOW_IS_SHOW_" + ID[Token[2]] +
                        " = false; \n");
      CPP_GLOBAL.append(" bool WINDOW_IS_CONSTRUCTION_" + ID[Token[2]] +
                        " = false; \n");
