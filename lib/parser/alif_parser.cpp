...
#include "parser/function_class.h"
#include "parser/if.h"
#include "parser/if_else.h"
#include "parser/int.h"
#include "parser/loop.h"
#include "parser/next_break.h"
#include "parser/obj.h"
#include "parser/obj_new.h"
#include "parser/operator.h"
#include "parser/return.h"
#include "parser/string.h"
#include "parser/var.h"
#include "parser/var_g.h"
  ...
  for (o_tokens->Line = 1; o_tokens->Line <= o_tokens->TOTAL_LINES;
    ...
    for (o_tokens->NUMBER = 1;
      ...
      if (o_tokens->TOKEN[std::make_pair(o_tokens->Line, o_tokens->NUMBER)] !=
        ...
        // o_tokens->Line, o_tokens->NUMBER, Token[o_tokens->NUMBER]);
                                  ...
                                  // "الف_اندرويد" || Token[2] == "الف_ايفون")
      {
        ...
        if (o_tokens->ALIF_FLAG)
        {
                if (ALIF_PROGRAM == "PC_BIN_32_64")
                        ErrorCode("Alif flag already set at line : " +
        o_tokens->ALIF_FLAG_AT_LINE + " (For PC) ", o_tokens);

                else if (ALIF_PROGRAM == "ANDROID")
                        ErrorCode("Alif flag already set at line : " +
        o_tokens->ALIF_FLAG_AT_LINE + " (For Android) ", o_tokens);

                else if (ALIF_PROGRAM == "IPHONE")
                        ErrorCode("Alif flag already set at line : " +
        o_tokens->ALIF_FLAG_AT_LINE + " (For iPhone) ", o_tokens);

                else
                        ErrorCode("علة: Unknow program type : ' " + ALIF_PROGRAM
        + " ' ", o_tokens);
        }


        if (ALIF_PROGRAM != "")
                ErrorCode("علة: Unknow program : ' " + ALIF_PROGRAM + " ' ",
        o_tokens);
        ...
        else if (Token[2] == "الف_ايفون")
        {
                ...
                ErrorCode("علة: Unknow flag type : ' " + Token[2] + " ' ",
        o_tokens);
        */
      }
      // --------------------------------------------
    ...
    */
    ...
    else if (CONTROL_WIN_IS_SET[Token[1]] || WIN_IS_SET[Token[1]] ||
    ...
    // #######################################################
      ...
      if (!o_tokens->TOKENS_PREDEFINED) {
        ...
        // نافذة
        else if (Token[1] == "نافدة" || Token[1] == "النافدة" ||
