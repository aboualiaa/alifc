...
void parser_Function(std::string Token[2048], lex::Token *o_tokens) {
  ...
  if (Token[FUN_POS + 1] == "رئيسية") // دالة رئيسية
  {
    ...
    Win_FunctionNames[std::make_pair(TheWindow, Win_CurrentTotalFucntion)] =
        TheFunction;
                                   ...
                                   // دالة Mouse_Click MyFunctionName ()
  {
    ...
    if (!o_tokens->TOKENS_PREDEFINED) {
      ...
      ADD_FUN(false, TheWindow, (Token[FUN_POS + 1] + " " + Token[FUN_POS + 2]),
              "عادم", o_tokens->Line, o_tokens);

      IsInsideFunction = true; // Need by Tokens Predefined
      TheFunction = (Token[FUN_POS + 1] + " " +
                     Token[FUN_POS + 2]); // Need by Tokens Predefined
    ...
    if (Token[FUN_POS + 1] == "نقر") // || Token[FUN_POS + 1] == "فارة_نقر")
    {
      ...
      CPP_WINDOW[std::make_pair(TheWindow, "FUN_DECLARATION")] =
          CBUFER + " void FUNCTION_MouseClick_" +
          Control_ID[Token[FUN_POS + 2]] + "(wxCommandEvent &event); \n";

      CBUFER = CPP_WINDOW[std::make_pair(TheWindow, "EVENT")];
      CPP_WINDOW[std::make_pair(TheWindow, "EVENT")] =
          CBUFER + " EVT_BUTTON(ID_CTR_" + ID[TheWindow] + "_" +
          Control_ID[Token[FUN_POS + 2]] + ", CLASS_WINDOW_" + ID[TheWindow] +
          "::FUNCTION_MouseClick_" + Control_ID[Token[FUN_POS + 2]] + ") \n";

      CPP_FUN.append(" void CLASS_WINDOW_" + ID[TheWindow] +
                     "::FUNCTION_MouseClick_" + Control_ID[Token[FUN_POS + 2]] +
                     "(wxCommandEvent &event){event.Skip(); \n");
      // *** *** *** *** *** ***
    } else {
      ErrorCode("علة: نوع الحدث غير معروف ' " + Token[FUN_POS + 1] + " ' ",
                o_tokens);
    }
    ...
    if (Token[FUN_POS + 3] == "") // دالة TYPE MyFunctionName
    {
      if (Token[FUN_POS + 1] == "عدد") {
        if (!IsInsideWindow) {
          ...
          if (!o_tokens->TOKENS_PREDEFINED) {
            // Local function
            ADD_FUN(false, TheWindow, Token[FUN_POS + 2], "عدد", o_tokens->Line,
                    o_tokens);
          ...
          CPP_FUN.append(" double CLASS_WINDOW_" + ID[TheWindow] +
                         "::FUNCTION_" + ID[Token[FUN_POS + 2]] + "(){ \n");
          // *** *** *** *** *** ***
        }
        ...
        if (!IsInsideWindow) {
          ...
          CPP_GLOBAL.append(" wxString FUNCTION_" +
                            Global_ID[Token[FUN_POS + 2]] + "(); \n");
          CPP_GLOBAL_FUN.append(" wxString FUNCTION_" +
                                Global_ID[Token[FUN_POS + 2]] + "(){ \n");
          // *** *** *** *** *** ***
        } else {
          if (!o_tokens->TOKENS_PREDEFINED) {
            // Local function
            ADD_FUN(false, TheWindow, Token[FUN_POS + 2], "نص", o_tokens->Line,
                    o_tokens);
          ...
          CBUFER = CPP_WINDOW[std::make_pair(TheWindow, "FUN_DECLARATION")];
          CPP_WINDOW[std::make_pair(TheWindow, "FUN_DECLARATION")] =
              CBUFER + " wxString FUNCTION_" + ID[Token[FUN_POS + 2]] +
              "(); \n";
          CPP_FUN.append(" wxString CLASS_WINDOW_" + ID[TheWindow] +
                         "::FUNCTION_" + ID[Token[FUN_POS + 2]] + "(){ \n");
          // *** *** *** *** *** ***
        }
        ...
        if (!IsInsideWindow) {
          ...
          if (!o_tokens->TOKENS_PREDEFINED) {
            // Local function
            ADD_FUN(false, TheWindow, Token[FUN_POS + 2], "منطق",
          ...
          CPP_FUN.append(" bool CLASS_WINDOW_" + ID[TheWindow] + "::FUNCTION_" +
                         ID[Token[FUN_POS + 2]] + "(){ \n");
          // *** *** *** *** *** ***
        }
      ...
      if (Token[FUN_POS + 4] == ")") // دالة TYPE MyFunctionName ()
      {
        ...
        if (Token[FUN_POS + 1] == "عدد") {
          if (!IsInsideWindow) {
            ...
            if (!o_tokens->TOKENS_PREDEFINED) {
              // Local function
              ADD_FUN(false, TheWindow, Token[FUN_POS + 2], "عدد",
                      o_tokens->Line, o_tokens);
          ...
          Win_FunctionNames[std::make_pair(
              TheWindow, Win_CurrentTotalFucntion)] = TheFunction;
          Win_CurrentTotalFucntion++;
          Win_TotalFucntion[TheWindow] = Win_CurrentTotalFucntion;
          ...
          if (!IsInsideWindow) {
            ...
            if (!o_tokens->TOKENS_PREDEFINED) {
              // Local function
              ADD_FUN(false, TheWindow, Token[FUN_POS + 2], "نص",
                      o_tokens->Line, o_tokens);
          ...
          if (!IsInsideWindow) {
            ...
            CPP_WINDOW[std::make_pair(TheWindow, "FUN_DECLARATION")] =
                CBUFER + " bool FUNCTION_" + ID[Token[FUN_POS + 2]] + "(); \n";
            CPP_FUN.append(" bool CLASS_WINDOW_" + ID[TheWindow] +
                           "::FUNCTION_" + ID[Token[FUN_POS + 2]] + "(){ \n");
            // *** *** *** *** *** ***
          }
  ...
  } else // void : دالة MyFunctionName (???) // void
  {
    ...
    if (Token[FUN_POS + 2] == "") // دالة MyFunctionName
    {
      if (!IsInsideWindow) {
        ...
        if (!o_tokens->TOKENS_PREDEFINED) {
          // Local function
          ADD_FUN(false, TheWindow, Token[FUN_POS + 1], "عادم", o_tokens->Line,
                  o_tokens);
        ...
        CPP_FUN.append(" void CLASS_WINDOW_" + ID[TheWindow] + "::FUNCTION_" +
                       ID[Token[FUN_POS + 1]] + "(){ \n");
        // *** *** *** *** *** ***
      }
    ...
    } else if (Token[FUN_POS + 2] == "(") // دالة MyFunctionName ( ???
    {
      if (Token[FUN_POS + 3] == ")") // دالة MyFunctionName ()
      {
        ...
        if (!IsInsideWindow) {
          ...
          if (!o_tokens->TOKENS_PREDEFINED) {
            // Local function
            ADD_FUN(false, TheWindow, Token[FUN_POS + 1], "عادم",
                    o_tokens->Line, o_tokens);
