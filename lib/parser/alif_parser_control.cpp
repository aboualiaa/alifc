...
void parser_Control(std::string Token[2048], CLASS_TOKEN *o_tokens) {
  ...
  if (Token[2] == "نافذة") {
    ...
    if (!o_tokens->TOKENS_PREDEFINED) {
      ...
      CPP_WINDOW[std::make_pair(Token[4], "شكل")] =
    ...
    CPP_WINDOW[std::make_pair(Token[4], "نص")] =
        GET_TXT_FROM_STRING(Token[14], o_tokens);
    if (Token[4] != "رئيسية") {
      WIN_TOTAL++;
      WIN_TOTAL_NAMES[WIN_TOTAL] = Token[4];
      CPP_ID_DECLARATION.append(" int ID_WINDOW_" + ID[Token[4]] +
                                " = ALIFCORE_ID_GENERATOR(); \n");
      CPP_GLOBAL.append(" bool WINDOW_IS_SHOW_" + ID[Token[4]] +
                        " = false; \n");
      CPP_GLOBAL.append(" bool WINDOW_IS_CONSTRUCTION_" + ID[Token[4]] +
                        " = false; \n");
    ...
    if (Token[2] == "زر") {
      ...
      CPP_WINDOW[std::make_pair(Token[4], "CTR_CONSTRUCTOR")] =
          CBUFER + " \n" + CBUFER_OBJ + " = new wxButton (P, " + CBUFER_ID +
          ", wxT(\"" + GET_TXT_FROM_STRING(Token[16], o_tokens) +
          "\"), wxPoint(" + Token[8] + "," + Token[10] + "), wxSize(" +
          Token[12] + ", " + Token[14] + ")); \n";
    } else if (Token[2] == "نص") {
      // TODO: AppendText -> أضف
      //       SetLabel -> نص
      ...
      CBUFER = CPP_WINDOW[std::make_pair(Token[4], "CTR_CONSTRUCTOR")];
      CPP_WINDOW[std::make_pair(Token[4], "CTR_CONSTRUCTOR")] =
          CBUFER + " \n" + CBUFER_OBJ + " = new wxTextCtrl (P, " + CBUFER_ID +
          ", wxT(\"" + GET_TXT_FROM_STRING(Token[16], o_tokens) +
          "\"), wxPoint(" + Token[8] + "," + Token[10] + "), wxSize(" +
          Token[12] + ", " + Token[14] + "), wxTE_MULTILINE | wxTE_RIGHT); \n";
    } else if (Token[2] == "ملصق") {
      CPP_OBJ_DECLARATION.append(" wxStaticText* " + CBUFER_OBJ + "; \n");
      CBUFER = CPP_WINDOW[std::make_pair(Token[4], "CTR_CONSTRUCTOR")];
      CPP_WINDOW[std::make_pair(Token[4], "CTR_CONSTRUCTOR")] =
          CBUFER + " \n" + CBUFER_OBJ + " = new wxStaticText (P, " + CBUFER_ID +
          ", wxT(\"" + GET_TXT_FROM_STRING(Token[16], o_tokens) +
          "\"), wxPoint(" + Token[8] + "," + Token[10] + "), wxSize(" +
          Token[12] + ", " + Token[14] +
